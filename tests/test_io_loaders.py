...
from facet.io.loaders import EDFLoader, _apply_sample_window
...
def test_edf_loader_applies_sample_window(monkeypatch, raw_factory):
    def fake_read_raw_edf(path, *args, **kwargs):
        return raw_factory()

    monkeypatch.setattr(mne.io, "read_raw_edf", fake_read_raw_edf)

    loader = EDFLoader(path="dummy.edf", start_sample=50, stop_sample=150)
...
def test_edf_loader_invalid_window_raises(monkeypatch, raw_factory):
    def fake_read_raw_edf(path, *args, **kwargs):
        return raw_factory()

    monkeypatch.setattr(mne.io, "read_raw_edf", fake_read_raw_edf)

    loader = EDFLoader(path="dummy.edf", start_sample=600, stop_sample=550)
