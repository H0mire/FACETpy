"""Smoke training test for DHCT-GAN v2 — runs a tiny in-memory training loop."""
...
from facet.models.masterthesis.dhct_gan.v2.training import build_dataset, build_loss, build_model
...
def _make_fixture_npz(path: Path, *, context: int = 7) -> None:
    ...
    n_examples, n_channels, samples = 8, 4, 64
...
def test_two_epoch_smoke_loop(tmp_path: Path) -> None:
    ...
    dataset = build_dataset(path=str(npz), context_epochs=7, demean_input=True, demean_target=True)
    train_ds, val_ds = dataset.train_val_split(val_ratio=0.25, seed=0)

    model = build_model(epoch_samples=64, context_epochs=7, base_channels=8, depth=3)
