...
def render_gallery(profiles):
    """Combine diagrams with the model README summaries and family links."""
    ...
    for profile in profiles:
        ...
        heading_ = profile["title"]
        directory = "../../" + profile["model_dir"] + "/diagrams"
        ...
        origin = re.search(r"^Origin: (.+)$", readme, re.MULTILINE)
        if reference is None or origin is None:
            raise ValueError(f"{readme_path}: missing model reference or origin")
        lines.extend(
            [
                f".. _{anchor}:",
                "",
                ...
                "",
                f":doc:`Model description and sources <{reference.group(1)}>`.",
                "",
                f".. figure:: {directory}/overview.svg",
                "   :width: 190mm",
                f"   :alt: {profile['title']} — compact model overview",
                "",
                f"   {profile['title']}: high-level processing steps.",
                "",
                f":download:`Detailed architecture (SVG) <{directory}/architecture.svg>`",
                "|",
                f":download:`Architecture profile (JSON) <{directory}/architecture.json>`",
                "",
            ]
        )
    return "\n".join(lines)
...
def main(argv=None):
    ...
    if not args.profiles:
        gallery = ROOT / "docs/source/model_diagrams.rst"
        content = render_gallery(profiles)
        if args.check:
            if not gallery.is_file() or gallery.read_text(encoding="utf-8") != content:
                failures.append(str(gallery.relative_to(ROOT)))
        else:
