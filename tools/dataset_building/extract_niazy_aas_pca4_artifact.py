...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["farm", "aas"],
        default="farm",
        help="Template selection. FARM (MATLAB FACET AvgArtWghtFARM) picks the most correlated "
        "epochs from a wide neighbourhood instead of a fixed sliding window, which tracks "
        "non-stationarity better; 'aas' keeps the plain sliding-window average.",
    )
    ...
    parser.add_argument("--farm-correlation-threshold", type=float, default=0.9, help="MATLAB FARM default")
...
def _template_processor(args):
    """AAS or FARM as the template stage (FARM subclasses AAS, so it is a drop-in)."""
    if args.method == "farm":
        return FARMCorrection(
            window_size=args.aas_window_size,
...
def main() -> None:
    ...
    metadata = {
        ...
        "source_type": f"{args.method}_plus_pca_obs_artifact_eeg_only_upsampled_no_lowpass",
        "template_method": args.method,
        "artifact_composition": f"{args.method.upper()} + PCACorrection(n_components={args.n_components}, hp_freq={args.hp_freq})",
