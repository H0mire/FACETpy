...
def extract_real_ied_pool(
    ...
    *,
    ...
    Scans the ``.mat`` recordings for ``!`` spike onsets and extracts a
    multi-channel window over the 19 standard 10-20 EEG channels (rows 0-18),
    1 Hz high-passed, **in the source's own volts — deliberately not normalised**
    (run_3 §6.6): the real morphology, the real cross-channel topography *and* the
    real amplitude are all preserved. Channels are mapped to the target montage by
    name at injection time; ``marker`` carries the ``!`` offset so the ground-truth
    label needs no detection.

    ``max_ieds`` defaults to ``None`` = load every annotated IED. A low cap
    silently restricts the pool to the first few recordings (the scan is
    file-ordered), which costs patient diversity — set one only for quick tests.
    """
    ...
    for f in mats:
        ...
        for on in onsets:
            ...
            if a < 0 or b > eeg.shape[1]:
                continue
            win = eeg[:, a:b]
            if not np.any(np.abs(win) > 0):
                continue
            # Kept in the source's own volts — VEPISET and the in-scanner recording
            # are both scalp EEG with a comparable background, so a real IED already
            # sits at a realistic level and must NOT be renormalised (run_3 §6.6).
            # ``marker`` = pool-sample index of the '!' onset (window centre).
            pool.append({"waveforms": win.astype(np.float32), "names": list(VEPISET_EEG19), "marker": int(half)})
            if max_ieds is not None and len(pool) >= max_ieds:
                break
        if max_ieds is not None and len(pool) >= max_ieds:
            break
    if not pool:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=float,
        default=0.2,
        help="Validation share. The split is contiguous in time AND the pre-trigger clean is cut "
        "disjointly, so the tiled clean cannot leak across the boundary (run_3 §7).",
    )
    p.add_argument(
        "--no-failure-modes",
        action="store_true",
...
def main() -> None:
    ...
    dataset = build_spatiotemporal_reference_dataset(
        ...
        spike_amplitude_scale=args.spike_amplitude_scale,
        spike_taper_s=args.spike_taper_s,
        val_fraction=args.val_fraction,
        failure_modes=not args.no_failure_modes,
        epoch_amplitude_jitter=args.epoch_amplitude_jitter,
        epoch_timing_jitter_samples=args.epoch_timing_jitter_samples,
        motion_drift_depth=args.motion_drift_depth,
        motion_drift_hz=args.motion_drift_hz,
        helium_pump_uv=args.helium_pump_uv,
        helium_pump_hz=args.helium_pump_hz,
