...
def main() -> int:
    ...
    if len(picks) == 0:
        picks = [i for i, n in enumerate(raw.ch_names) if n.upper() not in {'EKG', 'ECG', 'EMG', 'EOG', 'STATUS', 'MARKER'}]
    params = SpikeParams(spike_duration_ms=args.duration_ms, amplitude_variability=0.0, duration_variability=0.0, polyspike_probability=0.0)
    ...
    truth = {'input': str(args.input.relative_to(REPO)), 'sfreq_hz': sfreq, 'n_channels_injected': len(picks), 'amplitude_uv': args.amplitude, 'duration_ms': args.duration_ms, 'seed': args.seed, 'complex_samples': int(complex_v.size), 'centre_offset_samples': int(centre), 'spikes': placed, 'note': 'Gleiche Amplitude auf allen EEG-Kanälen — bewusst unphysiologisch, siehe Modul-Docstring.'}
    ...
    print(f'{len(placed)} Spikes gesetzt auf {len(picks)} Kanälen, Spitzenwert {peak:.1f} µV, Länge {complex_v.size / sfreq * 1000:.0f} ms')
    for p in placed:
