"""Pipeline adapters for every evaluated model family.

Until now only the cascade and the Weg-A direct models could run inside a real
FACETpy pipeline; the fourteen models of the unified holdout existed only as
inference functions over prepared NPZ tensors. That is a gap in the thesis story
— "this architecture corrects EEG" is a weaker claim if the architecture has
never seen an EDF.

The fourteen contracts differ, but not arbitrarily. They factor into four
independent choices, and this module encodes exactly those:

``context``
    ``single`` — only the centre epoch, or ``stack`` — the seven-epoch context.
``packing``
    the tensor shape the export expects.
``demean``
    which axis the mean is removed over. Getting this wrong does not raise, it
    silently shifts the prediction, which is why it is a named field and not an
    inline expression.
``output``
    ``artifact`` directly, ``clean`` (artifact is then ``noisy − prediction``),
    or ``sources`` where index 1 is the artifact.
...
class PackingSpec:
    ...
    batch_size: int = 32
    note: str = ""
    #: Glob for the TorchScript export, relative to ``training_output``. Empty
    #: means "look the model up in ``eval_unified_holdout.MODELS``", which is
    #: where the fourteen original runs live. The deployment editions are not in
    #: that registry on purpose: it is the frozen record of the run the evidence
    #: pack cites, and adding rows to it would change what those claims refer to.
    ts_glob: str = ""
...
DEPLOYMENT_SPECS: dict[str, PackingSpec] = {
    f"{model_id}_deployment": PackingSpec(
        ...
        ts_glob=(f"{model_id.replace('_', '')}deploymentniazyprooffit_*"
                 f"/exports/{model_id}_deployment.ts"),
    )
...
def load_model(model_id: str, device: str):
    """Load one family's trained model, exactly as the holdout evaluator does.

    Three loaders, because three exports behave differently:

    * **TorchScript** for most, resolving the registry's glob pattern first.
    * **Python source** for ``denoise_mamba``: its TorchScript export has a CUDA
      device baked into the SSM scan, so it cannot run on CPU or MPS at all.
    * **Training module** for ``d4pm``: the shipped ``.ts`` is a stub without the
      DDPM sampler, so the reverse loop has to be reconstructed.

    Kept here rather than in the verification script so the pipeline adapter and
    the verification use one loader; two loaders would be two contracts.
    """
    import sys
    ...
    if model_id in DEPLOYMENT_SPECS:
        packing = DEPLOYMENT_SPECS[model_id]
        matches = sorted(TRAIN_ROOT.glob(packing.ts_glob))
        if not matches:
            raise FileNotFoundError(
                f"{model_id}: no export matching {packing.ts_glob} under {TRAIN_ROOT}. "
                f"Train it with "
                f"src/facet/models/{model_id[:-len('_deployment')]}_deployment_edition/"
                f"training_niazy_proof_fit.yaml first.")
        return _load_torchscript(matches[-1], device)
    ...
    if packing.loader == "d4pm":
        ...
        module = D4PMTrainingModule(epoch_samples=512, num_steps=200, beta_start=1e-4,
                                    ...
                                    n_heads=2, n_layers=2, embed_dim=128)
        state = torch.load(str(ckpt), map_location="cpu", weights_only=False)
        module.load_state_dict(state.get("model_state_dict", state), strict=True)
        return module.float().to(device).eval()
    ts = _resolve_ts_path(spec.ts_path)
    # Prefer the CPU-baked export on any non-CUDA device, not only on CPU.
    # Some traces contain a hard-coded CUDA device (aten::empty_strided on the
    # CUDA backend), so the default export cannot run on MPS either — the
    # _cpu.ts variants exist precisely because of that.
    if device != "cuda" and spec.ts_path_cpu:
        cpu_variant = _resolve_ts_path(spec.ts_path_cpu)
        if cpu_variant and (TRAIN_ROOT / cpu_variant).exists():
            ts = cpu_variant
    if device == "cpu":
        return _load_torchscript(TRAIN_ROOT / ts, device)
    # MPS cannot deserialise these archives directly — torch.jit.load with
    # map_location="mps" raises on a float64 constant inside the archive, even
    # though no parameter or buffer is float64. Loading on CPU, casting to
    # float32 and then moving works, and is the only route to GPU here.
    import torch
    model = torch.jit.load(str(TRAIN_ROOT / ts), map_location="cpu")
    return model.float().to(device).eval()
    ...
    * **Edge epochs.** Missing context epochs are mirrored about the centre, the
    ...
    def _load_model(self):
        ...
        if self._model is None:
            self._model = load_model(self.model_id, self.device)
