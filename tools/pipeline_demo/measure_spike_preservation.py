...
def main() -> int:
    ...
    for arm in arms:
        ...
        rows.append({'arm': arm, **{f'spike_{t:g}s_uv': round(p, 2) for t, p in zip(times, peaks, strict=True)}, 'mittel_uv': round(float(np.mean(peaks)), 2), 'erhalt_pct': round(100.0 * float(np.mean(peaks)) / injected, 1)})
    ...
    with open(args.out, 'w', newline='') as fh:
        ...
        w.writeheader()
        w.writerows(rows)
    print(f"Injected: {injected:.0f} µV; measured at {args.channel}, {len(times)} locations ({', '.join((f'{t:g} s' for t in times))})\n")
    ...
    for r in rows:
        ...
        print(f"{r['arm']:32s}{vals}{r['mittel_uv']:9.1f}{r['erhalt_pct']:8.1f}%{(f'{gegen:9.1f}%' if gegen is not None else '        -')}")
    print(f'\nWritten: {args.out}')
    return 0
