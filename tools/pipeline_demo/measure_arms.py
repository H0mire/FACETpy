...
def measure(path: Path, t0: float, t1: float, step_channel: str) -> dict:
    ...
    comb = (
        GradientArtifactResidualCalculator(tmin=t0, tmax=t1).execute(ctx).metadata.custom["gradient_artifact_residual"]
    )
...
def main() -> int:
    ...
    for r in rows:
        ...
        r["hinweis"] = (
            "Invalid seam; inspect waveform"
            if r["naht_ratio"] >= SEAM_BROKEN
            else "Suspicious seam"
            if r["naht_ratio"] >= SEAM_SUSPECT
            else ""
        )
    ...
    meta = {
        ...
        "step_channel": args.channel,
        "metrics": {
            "ga_rest_uv": "GradientArtifactResidualCalculator.comb_rms_uv, fmax 70 Hz",
            "naht_ratio": "EpochSeamStepCalculator.ratio",
            "median_sample_step_uv": "Median absolute adjacent-sample difference on the selected channel.",
        },
        "caveat": "The comb residual depends on the time window and measures only periodic residuals. Interpret it together with the seam score and waveform.",
    }
    ...
    for r in rows:
        print(
            f"{r['arm']:32s}{r['rms_uv']:8.2f}{r['ga_rest_uv']:9.2f}{r['x_reference']:7.1f}{r['naht_ratio']:7.2f}  {r['hinweis']}"
        )
    print(f"\nWritten: {args.out}\n             {meta_path}")
    return 0
