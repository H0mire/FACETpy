...
def main() -> None:
    ...
    model_ids = args.models or sorted(
        e.removeprefix("holdout_")
        for e, record in catalog["experiments"].items()
        if e.startswith("holdout_") and record["artifacts"]
    )
    ...
    for model_id in model_ids:
        ...
        print(
            f"  {model_id:24s} {rows[-1]['inference_seconds_mean']:7.3f} ± "
            f"{rows[-1]['inference_seconds_sd']:.3f} s   "
            f"{rows[-1]['peak_rss_mib_mean']:7.0f} MiB",
            flush=True,
        )
    ...
    (args.out / "benchmark_models.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"\n{len(rows)} models measured, {len(failures)} unavailable -> {args.out / 'benchmark_models.json'}")
