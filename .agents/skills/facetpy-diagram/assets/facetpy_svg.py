...
-----------
    ...
    d = Diagram(660, title="Restaurant", subtitle="domain model")
...
def _node_dot(cx, cy):
    ...
    return (
        f'<circle cx="{cx}" cy="{cy}" r="4.5" fill="{C["blue200"]}" '
        f'stroke="#ffffff" stroke-opacity="0.85" stroke-width="1.2"/>'
    )


def _text(x, y, s, size=12.5, fill=None, weight=None, anchor="start", italic=False, opacity=None):
    fill = fill or C["ink"]
    extra = ""
    if weight:
    ...
    return f'<text x="{x}" y="{y}" font-family="{FONT}" font-size="{size}" fill="{fill}"{extra}>{esc(s)}</text>'
...
def _header(x, y, w, name, stereotype=None):
    ...
    p = [f'<path d="{_rounded_top(x, y, w, HEAD, 12)}" fill="url(#fp-header)"/>', _node_dot(x + 18, y + HEAD / 2)]
    if stereotype:
        p.append(
            _text(x + 34, y + HEAD / 2 - 4, f"«{stereotype}»", size=10.5, fill=C["header_fg"], italic=True, opacity=0.7)
        )
        p.append(_text(x + 34, y + HEAD / 2 + 11, name, size=15, fill=C["header_fg"], weight=600))
    else:
        p.append(_text(x + 34, y + HEAD / 2 + 5, name, size=15.5, fill=C["header_fg"], weight=600))
    return "".join(p)
...
def _divider(x, y, w):
    return f'<line x1="{x}" y1="{y}" x2="{x + w}" y2="{y}" stroke="{C["ink"]}" stroke-opacity="0.12"/>'
...
def _member(x, y, w, vis, name, typ, kind="attr"):
    ...
    s = (
        f'<text x="{x + 14}" y="{y}" font-family="{FONT}" font-size="12.5" '
        f'fill="{C["ink"]}">'
...
def class_box(x, y, w, name, attrs, methods, stereotype=None):
    ...
    for vis, mname, typ in attrs:
        p.append(_member(x, ay, w, vis, mname, typ))
        ay += ROW
    dy = cy + attr_h
    p.append(_divider(x, dy, w))
    my = dy + PAD + 15
    for vis, mname, typ in methods:
        p.append(_member(x, my, w, vis, mname, typ, kind="method"))
        my += ROW
...
def card(x, y, w, h, title, lines=None, stereotype=None):
    ...
    if lines:
        ...
        ly = y + HEAD + PAD + 15
        for ln in lines:
            p.append(_text(x + 16, ly, ln, size=12.5, fill=C["slate"]))
            ly += ROW
    return _geom("".join(p), x, y, w, h)
...
def pill(x, y, w, h, text, accent=None):
    ...
    svg = (
        f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="{h / 2 if h < 40 else 12}" '
        ...
        + _text(x + w / 2, y + h / 2 + 4.5, text, size=13, weight=500, anchor="middle")
    )
...
def capsule(x, y, w, h, text):
    ...
    svg = (
        ...
        + _text(x + w / 2, y + h / 2 + 4.5, text, size=13, weight=600, fill=C["header_fg"], anchor="middle")
    )
    return _geom(svg, x, y, w, h)
...
def decision(cx, cy, w, h, text):
    ...
    svg = (
        f'<polygon points="{pts}" fill="{C["surface"]}" stroke="{C["blue"]}" '
        f'stroke-width="1.5" filter="url(#fp-shadow)"/>'
...
def container(x, y, w, h, title):
    ...
    svg = (
        ...
        + _text(x + 14, y + 17.5, title, size=12.5, weight=600, fill=C["header_fg"])
    )
    return _geom(svg, x, y, w, h)
...
def lifeline(cx, top, bottom, name, w=140, head=HEAD):
    ...
    x = cx - w / 2
    svg = (
        f'<path d="M{cx} {top + head} L{cx} {bottom}" stroke="{C["ink"]}" '
        ...
        + _text(cx + 6, top + head / 2 + 5, name, size=13.5, weight=600, fill=C["header_fg"], anchor="middle")
    )
...
def activation(cx, y1, y2, w=10):
    return (
        f'<rect x="{cx - w / 2}" y="{y1}" width="{w}" height="{y2 - y1}" rx="2" '
...
def message(x1, x2, y, text, dashed=False, ret=False):
    ...
    line = (
        f'<line x1="{x1}" y1="{y}" x2="{x2}" y2="{y}" stroke="{C["ink"]}" '
        f'stroke-opacity="0.6" stroke-width="1.5" marker-end="url(#{marker})"{dash}/>'
    )
    lx = (x1 + x2) / 2
    return line + _text(lx, y - 7, text, size=11.5, fill=C["blue"], weight=500, anchor="middle")
...
def edge(points, marker_end="arrow", marker_start=None, label=None, mult=None):
    ...
    if marker_end:
        a += f' marker-end="url(#fp-{marker_end})"'
    if marker_start:
        a += f' marker-start="url(#fp-{marker_start})"'
    out = [f'<polyline points="{pstr}" {a}/>']
    if label:
        txt, lx, ly = label
        bw = _text_w(txt, 11) + 14  # chip auto-sizes to the text
        out.append(
            f'<rect x="{lx - bw / 2:.1f}" y="{ly - 12}" width="{bw:.1f}" '
            f'height="17" rx="4" fill="{C["tint"]}" stroke="{C["ink"]}" '
            f'stroke-opacity="0.10"/>'
        )
        out.append(_text(lx, ly, txt, size=11, fill=C["blue"], weight=500, anchor="middle"))
...
def eeg_wave(x, y, w, color=None, opacity=0.9):
    ...
    for i in range(n + 1):
        ...
        if t < 0.25:
            ...
            py = y - 6 * math.sin((t - 0.25) / 0.30 * math.pi * 3)
        elif t < 0.68:
            py = y - 26 * math.sin((t - 0.55) / 0.13 * math.pi)
        else:
            py = y
...
def title_block(title, subtitle=None, x=70, y=52):
    p = [f'<text x="{x}" y="{y}" font-family="{FONT}" font-size="24" font-weight="700" fill="{C["ink"]}">{esc(title)}']
    if subtitle:
        p.append(f' <tspan font-weight="400" fill="{C["slate"]}" font-size="15">· {esc(subtitle)}</tspan>')
    p.append("</text>")
    uw = 22 + len(title) * 9
    p.append(f'<rect x="{x}" y="{y + 10}" width="{uw}" height="3" rx="1.5" fill="url(#fp-header)"/>')
...
def footer(W, H, text="Made with FACETpy"):
    return _text(W - 20, H - 18, text, size=11, fill=C["slate"], anchor="end", opacity=0.8)
...
class Diagram:
    ...
    def render_png(self, png_path, svg_path=None, width=1280) -> str:
        ...
        if not exe:
            raise RuntimeError(
                "rsvg-convert not found — install librsvg (`brew install librsvg`) or render the SVG another way."
            )
