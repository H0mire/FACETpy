...
def _loss_terms(loss_fn: Any) -> dict[str, float]:
    """Per-term diagnostics a composite loss chose to expose.

    Until now the training log carried a single number per epoch, which is how a
    model can spend 120 epochs deleting the EEG without anything in the log
    saying so. A loss module that sets ``last_terms`` gets those values logged,
    ranked and plotted alongside ``loss`` for free.
    """
    terms = getattr(loss_fn, "last_terms", None)
...
class PyTorchModelWrapper(TrainableModelWrapper):
    ...
    def train_step(self, noisy: np.ndarray, target: np.ndarray) -> dict[str, float]:
        ...
        return {"loss": float(loss.detach().cpu()), **_loss_terms(self._loss_fn)}
    ...
    def eval_step(self, noisy: np.ndarray, target: np.ndarray) -> dict[str, float]:
        ...
        return {"loss": float(loss.cpu()), **_loss_terms(self._loss_fn)}
