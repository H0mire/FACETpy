...
class EarlyStoppingConfig:
    ...
    min_delta : float
        Minimum **absolute** change in *monitor* to count as an improvement.
    min_delta_rel : float
        Minimum change as a **fraction of the current best**. The effective
        threshold is ``max(min_delta, min_delta_rel * |best|)``.

        This exists because an absolute threshold silently becomes either a
        no-op or a hard stop depending on the loss scale, and nothing warns you
        which. The ``vit_spectrogram`` run was configured with
        ``min_delta: 1.0e-06`` against an objective whose entire value was
        2.45e-07 -- no improvement could ever clear the bar, and the run stopped
        at epoch 5 of 120 having learned to output zero. Setting
        ``min_delta_rel`` instead makes the criterion scale-free.
    """
