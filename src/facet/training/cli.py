...
class ModelCLIConfig:
    ...
    # Escape hatch for models the one-optimizer/one-loss contract cannot express —
    # adversarial training above all. The factory receives the built model plus the
    # resolved training settings and returns a TrainableModelWrapper; see
    # facet.training.adversarial.AdversarialModelWrapper.
    wrapper_factory: str | None = None
...
def run_fit_command(config_path: str | Path) -> CLITrainingRun:
    ...
    try:
        ...
        # An empty split used to surface as "not enough values to unpack" from deep
        # inside the batch loop. It is almost always caused by combining a dataset
        # that stores its own split with a subsampling limit that lands entirely on
        # one side of the boundary, so say that here.
        if len(train_dataset) == 0:
            raise ProcessorValidationError(
                f"The training split is empty (val_ratio={cli_config.training.val_ratio}, "
                f"{len(dataset)} examples). Check data.kwargs.max_examples and the dataset's own split."
            )
        if val_dataset is not None and len(val_dataset) == 0:
            raise ProcessorValidationError(
                f"The validation split is empty (val_ratio={cli_config.training.val_ratio}, "
                f"{len(dataset)} examples). If the dataset stores its own train/val split, a "
                "subsampling limit can land entirely inside the training partition."
            )
        ...
        wrapper = _build_wrapper(cli_config, model, dataset=dataset, sfreq=sfreq)
...
def _build_wrapper(
    ...
    # Loss factories get the same run metadata the model factory gets. Without
    # this a loss that needs the sampling rate silently degrades: IC-U-Net's
    # frequency term reads ``sfreq`` to restrict itself to 1-50 Hz and, receiving
    # None, optimised the full spectrum instead of the documented band. As with
    # every factory, _invoke_factory drops what the signature does not declare.
    loss_injected = {
        "sfreq": sfreq,
    ...
    if cli_config.model.wrapper_factory:
        # A custom wrapper owns its own optimizers and loss, so the generic
        # optimizer/loss factories are passed as *offers*: _invoke_factory drops
        # whatever the signature does not declare.
        loss_fn = None
        if cli_config.model.loss_factory:
            loss_fn = _invoke_factory(
                _import_object(cli_config.model.loss_factory), cli_config.model.loss_kwargs, loss_injected
            )
        wrapper = _invoke_factory(
            _import_object(cli_config.model.wrapper_factory),
            cli_config.model.wrapper_kwargs,
            {
                "model": model,
                "loss_fn": loss_fn,
        ...
        if not isinstance(wrapper, TrainableModelWrapper):
            raise ProcessorValidationError(
                f"model.wrapper_factory '{cli_config.model.wrapper_factory}' returned "
                f"{type(wrapper).__name__}; it must return a TrainableModelWrapper."
            )
        return wrapper
    ...
    if framework == "pytorch":
        ...
        loss_fn = _invoke_factory(loss_factory, cli_config.model.loss_kwargs, loss_injected)
...
def _export_pytorch_torchscript(
    ...
    # Trace on the CPU, whatever the model was trained on. torch.jit.trace bakes
    # the device of every operation into the graph, so a model traced on CUDA
    # produces an archive that raises "Could not run 'aten::empty.memory_format'
    # with arguments from the 'CUDA' backend" on any machine without a GPU --
    # ``map_location="cpu"`` at load time does not help, because it remaps the
    # parameters and not the baked-in operations. That is an export nobody but
    # the training host can open, which defeats the point of exporting. A copy,
    # so moving it does not disturb a wrapper the caller may still use.
    device = torch.device("cpu")

    # A model with several heads cannot be traced as-is — TorchScript needs one
    # tensor out — and it is the model, not the exporter, that knows which head is
    # the deployed one. ``export_module()`` is that hook; without it the forward
    # pass is exported unchanged, which is the previous behaviour.
    export_target = model
    export_hook = getattr(model, "export_module", None)
    if callable(export_hook):
        export_target = export_hook()
        logger.info(f"Exporting via {type(model).__name__}.export_module() -> {type(export_target).__name__}")
    export_target = copy.deepcopy(export_target).to(device)

    example_input = torch.randn(*shape, device=device)
    # Trace with autograd off. ``nn.MultiheadAttention`` takes a different internal
    # path depending on whether grad is enabled, so tracing with grad on makes
    # torch.jit's own graph-consistency check fail on any attention model — a
    # spurious failure that has nothing to do with the exported computation.
    with torch.no_grad():
        scripted = torch.jit.trace(export_target.eval(), example_input)
...
def _describe_model(wrapper: Any) -> dict[str, Any] | None:
    """Collect self-reported model facts for ``summary.json``.

    Reads two optional hooks: ``model.describe()`` for architecture/configuration
    facts, and ``wrapper.parameter_counts()`` for the per-network parameter counts
    an adversarial run needs. Both are optional; a model that offers neither is
    simply not described.
    """
    if wrapper is None:
        return None
    described: dict[str, Any] = {}
    describe = getattr(getattr(wrapper, "model", None), "describe", None)
    if callable(describe):
        with contextlib.suppress(Exception):
            described.update(describe())
    counts = getattr(wrapper, "parameter_counts", None)
    if callable(counts):
        with contextlib.suppress(Exception):
            described["parameter_counts"] = counts()
...
def _write_run_summary(
    ...
    summary = {
        ...
        # What the model says about itself. Configuration facts that decide how a
        # result may be reported -- which decoder head, whether the multi-channel
        # extension was active, how many parameters per network -- belong in the
        # run directory, not only in the operator's memory.
        "model": _describe_model(wrapper),
        "export": {
