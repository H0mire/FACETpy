...
class Trainer:
    ...
    def _update_best(self, state: TrainingState) -> None:
        ...
        # Same prefixed/bare name resolution the callbacks use: the metric dicts
        # key plainly ("loss") while configs and the JSONL log speak "val_loss".
        value = _resolve_monitored(state, monitor)
        ...
        if not has_early_stopping and self.config.early_stopping is not None:
            callbacks.append(
                EarlyStoppingCallback(
                    ...
                    min_delta_rel=self.config.early_stopping.min_delta_rel,
                )
            )
