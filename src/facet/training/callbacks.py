...
def _resolve_monitored(state: TrainingState, monitor: str) -> float | None:
    """Look up ``monitor`` in a state's metrics, accepting prefixed names.

    ``state.train_metrics``/``val_metrics`` key their entries plainly (``"loss"``),
    but the JSONL log — and therefore every metric name a user actually sees —
    prefixes them (``"train_loss"``, ``"val_loss"``). Configs written against the
    visible names (as the class docstrings here recommend) silently monitored
    nothing, which disabled checkpoint ranking and early stopping altogether.

    Both spellings resolve here. ``val_*`` wins over ``train_*`` for a bare name,
    matching the previous merge order.
    """
    prefixed: dict[str, float] = {}
    prefixed.update({f"train_{k}": v for k, v in state.train_metrics.items()})
    prefixed.update({f"val_{k}": v for k, v in state.val_metrics.items()})
    if monitor in prefixed:
        return prefixed[monitor]
    plain = {**state.train_metrics, **state.val_metrics}
    return plain.get(monitor)
...
class CheckpointCallback(Callback):
    ...
    def on_epoch_end(self, state: TrainingState) -> None:
        value = _resolve_monitored(state, self.monitor)
...
class EarlyStoppingCallback(Callback):
    ...
    min_delta_rel : float
        Minimum change as a fraction of the current best value. The effective
        threshold is ``max(min_delta, min_delta_rel * |best|)``, so the criterion
        follows the loss scale instead of assuming one. An absolute threshold
        larger than the whole loss stops training on epoch ``patience``; see
        :class:`facet.training.config.EarlyStoppingConfig` for the run where that
        happened.
    verbose : bool
    ...
    def _threshold(self, best: float) -> float:
        """How much better the metric has to get, at the current loss scale."""
        if not self.min_delta_rel or not np.isfinite(best):
            return self.min_delta
        return max(self.min_delta, self.min_delta_rel * abs(best))

    def _is_better(self, new: float, best: float) -> bool:
        delta = self._threshold(best)
        return new < best - delta if self.mode == "min" else new > best + delta
...
def _to_1d(arr: np.ndarray) -> np.ndarray:
    ...
    Takes the first element of every leading axis, so a multi-channel array
    plots channel 0 and a row-stacked target plots its primary row.

    The loop matters. The previous version squeezed singleton axes and then
    peeled exactly *one* more, which is enough for rank 2 and silently leaves
    rank 3 two-dimensional: a ``(3, 1, 512)`` target came back as ``(1, 512)``
    and matplotlib raised "x and y must have same first dimension" *after*
    training had finished, taking the TorchScript export down with it. Rank 3 is
    ordinary now — losses that need the clean and noisy signals get them as extra
    target rows (:mod:`facet.training.deployment_losses`).
    """
    a = np.asarray(arr)
    while a.ndim > 1:
        a = a[0]
