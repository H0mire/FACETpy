"""Inference integration for the Dual-Pathway Autoencoder (DPAE)."""
...
class DualPathwayAutoencoderAdapter(EpochContextArtifactAdapter):
    """TorchScript adapter that runs DPAE per channel and per trigger epoch.

    The Niazy proof-fit dataset resamples each native trigger-to-trigger epoch
    to a fixed model length (default 512 samples). At inference we mirror that:
    each native epoch is resampled to ``epoch_samples``, fed through DPAE, then
    the predicted artifact is resampled back to the native epoch length.
    """

    spec = DeepLearningModelSpec(
        name="DualPathwayAutoencoderAdapter",
        ...
        uses_triggers=True,
        description="Channel-wise dual-pathway autoencoder artifact predictor.",
        tags=("dpae", "dual_pathway_autoencoder", "torchscript", "artifact_prediction"),
    )
    ...
    def validate_context(self, context: ProcessingContext) -> None:
        ...
        if self.epoch_samples % 4 != 0:
            raise ProcessorValidationError(
                f"epoch_samples must be divisible by 4 for the dual-pathway encoder, got {self.epoch_samples}"
            )
        ...
        with torch.no_grad():
            for epoch_start, epoch_stop in zip(starts, stops, strict=False):
                ...
                for ch_idx in channels:
                    ...
                    prediction = self._predict_segment(model, torch, resampled)
                    artifact_native = _resample_1d(prediction, epoch_len).astype(data.dtype, copy=False)
    ...
    def _predict_segment(self, model: Any, torch: Any, segment: np.ndarray) -> np.ndarray:
        if self.demean_input:
            segment = segment - np.mean(segment, dtype=np.float32)
...
@register_processor
class DualPathwayAutoencoderCorrection(DeepLearningCorrection):
    """Pipeline processor for DPAE inference."""

    name = "dpae_correction"
    description = "Channel-wise dual-pathway autoencoder artifact correction"
    version = "0.1.0"
