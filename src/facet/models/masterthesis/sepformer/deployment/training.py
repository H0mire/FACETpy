"""SepFormer deployment edition.

**What the base edition does on a real recording.** 22.8 µV residual, 2.62x FARM in the EEG band — the artifact is essentially uncorrected.
...
imported unchanged from :mod:`facet.models.masterthesis.sepformer.training`.
...
The base edition optimises SI-SNR, which is scale-invariant, so nothing in its objective pins the absolute µV the subtraction depends on. Here SI-SDR is one term of five and the four normalised time- and frequency-domain terms carry the scale.
...
from facet.models.masterthesis.sepformer.training import build_model as _build_core
