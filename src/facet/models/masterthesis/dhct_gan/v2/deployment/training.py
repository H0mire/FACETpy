"""DHCT-GAN v2 deployment edition.

**What the base edition does on a real recording.** 208 µV residual, 310x FARM in the EEG band. Best epoch 1 of 34.
...
imported unchanged from :mod:`facet.models.masterthesis.dhct_gan.v2.training`.
...
Checkpointing and early stopping move to ``val_loss``, for the same reason as ``dhct_gan``.

Packing ``bts`` and core output ``artifact`` are the base edition's,
...
from facet.models.masterthesis.dhct_gan.v2.training import build_model as _build_core
