"""DHCT-GAN deployment edition.

**What the base edition does on a real recording.** 1327 µV residual, 4981x FARM in the EEG band. Best epoch 1 of 16 — it diverged immediately.
...
imported unchanged from :mod:`facet.models.masterthesis.dhct_gan.training`.
...
Checkpointing and early stopping move to ``val_loss``. The base run ranked on the training loss, so its best epoch was the first one and the 15 that followed were never eligible.
The adversarial half of Cai et al. is a separate concern and lives in ``dhct_gan_strict_edition``; this edition changes the objective, not the architecture.
...
from facet.models.masterthesis.dhct_gan.training import build_model as _build_core
