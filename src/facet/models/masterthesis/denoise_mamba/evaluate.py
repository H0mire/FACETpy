"""Evaluate a trained DenoiseMamba checkpoint on the Niazy proof-fit context dataset.

Single-purpose evaluation that mirrors the metric set used by
``cascaded_context_dae`` and ``dpae`` so DenoiseMamba can be compared
row-for-row in ``output/model_evaluations/<model>/<run>/metrics.json``.

Example:
    uv run python -m facet.models.masterthesis.denoise_mamba.evaluate \
        --checkpoint training_output/denoisemambaniazyprooffit_<run>/exports/denoise_mamba.ts \
...
def _plot_examples(
    *,
    ...
    fig.suptitle("DenoiseMamba proof-fit cleaning examples (Niazy)", fontsize=11)
...
def main() -> None:
    ...
    writer = ModelEvaluationWriter(
        ...
        model_description="ConvSSD state-space gradient artifact denoiser (Section 6.2, Mamba-1 selective scan in pure PyTorch).",
    ...
    interpretation = (
        f"DenoiseMamba evaluated on the Niazy proof-fit context dataset "
