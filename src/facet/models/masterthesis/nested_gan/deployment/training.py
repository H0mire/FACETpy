"""Nested-GAN deployment edition.

**What the base edition does on a real recording.** 29.3 µV residual and 96 % of the output was a constant offset.
...
imported unchanged from :mod:`facet.models.masterthesis.nested_gan.training`.
...
The constant offset is fixed structurally by the per-epoch demean rather than by the inference adapter.
Yang et al. (Biomed. Phys. Eng. Express 11:065054, 2025) reach MSE 0.098 / PCC 89.2 % / RRMSE 0.065 with four discriminators and a gradient-balance step that we do not implement; this edition keeps the base generator and changes only what it is scored against.
...
from facet.models.masterthesis.nested_gan.training import build_model as _build_core
