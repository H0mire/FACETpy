"""Inference integration for the ViT Spectrogram Inpainter (`vit_spectrogram`)."""
...
class ViTSpectrogramInpainterAdapter(EpochContextArtifactAdapter):
    """TorchScript adapter that runs the ViT spectrogram inpainter per channel.

    Predicts the *clean* center epoch from a 7-epoch trigger-defined context
    and converts that to an artifact estimate via ``noisy - clean``. Mirrors
    the channel-wise inference pattern of
    :class:`facet.models.masterthesis.cascaded_context_dae.CascadedContextDenoisingAutoencoderAdapter`
    but operates in the spectrogram domain inside the TorchScript graph.
    """

    spec = DeepLearningModelSpec(
        name="ViTSpectrogramInpainterAdapter",
        ...
        uses_triggers=True,
        description="Vision-Transformer spectrogram inpainter for fMRI gradient artifact removal.",
        tags=(
            "vit_spectrogram",
            "vit",
            "mae",
    ...
    def _load_model(self) -> tuple[Any, Any]:
        ...
        try:
            ...
            raise ProcessorValidationError(
                "ViTSpectrogramInpainter requires PyTorch. Install the pytorch extra first."
            ) from exc
...
@register_processor
class ViTSpectrogramInpainterCorrection(DeepLearningCorrection):
    """Pipeline processor for ViT-based spectrogram-inpainting artifact correction."""

    name = "vit_spectrogram_correction"
    description = "Vision-Transformer spectrogram-inpainting artifact correction"
    version = "0.1.0"
    ...
    def __init__(
        ...
        *,
        ...
        adapter = ViTSpectrogramInpainterAdapter(
