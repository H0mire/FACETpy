"""ST-GNN deployment edition.

**What the base edition does on a real recording.** 8.63 µV residual, 96 % constant offset, epoch-seam step ratio 6.2 against a working corrector's 1.0.
...
imported unchanged from :mod:`facet.models.masterthesis.st_gnn.training`.
...
Both the offset and the seams are fixed structurally by the per-epoch demean.
The temporal receptive field remains a separate issue: ``time_kernel: 3`` is inherited from STGCN, where one step is a five-minute traffic observation, and here it spans 11 of 3584 samples — 2.7 ms. Widening it is a distinct experiment and is not bundled into this one.

Packing ``btcs`` and core output ``artifact`` are the base edition's,
...
from facet.models.masterthesis.st_gnn.training import build_model as _build_core
