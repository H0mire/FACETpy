"""Demucs deployment edition.

**What the base edition does on a real recording.** 12.69 µV residual, 1.62x FARM in the EEG band — visible artifact remnants.
...
imported unchanged from :mod:`facet.models.masterthesis.demucs.training`.
...
The best of the fourteen on the prepared holdout (RRMSE_t 0.1008, CC 0.9949) and still only middling through a pipeline, which is the gap this edition exists to close.
SPIE 1309027 uses encoder/decoder depth **2** where we use 4; that comparison is left as a separate config rather than folded in here, so this edition isolates the objective.

Packing ``b1ts`` and core output ``artifact`` are the base edition's,
...
from facet.models.masterthesis.demucs.training import build_model as _build_core
