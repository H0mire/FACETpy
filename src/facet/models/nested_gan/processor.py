"""Inference integration for the Nested-GAN generator on 7-epoch context input."""
...
class NestedGANAdapter(EpochContextArtifactAdapter):
    """TorchScript adapter that runs the Nested-GAN generator per channel per window."""

    spec = DeepLearningModelSpec(
        name="NestedGANAdapter",
        ...
        uses_triggers=True,
        description="Nested-GAN generator: inner spectrogram Restormer + outer time-domain refiner.",
        tags=(
            "nested_gan",
            "gan",
            "spectrogram",
    ...
    def _load_model(self) -> tuple[Any, Any]:
        ...
        try:
            ...
            raise ProcessorValidationError("NestedGAN requires PyTorch. Install the pytorch extra first.") from exc
...
@register_processor
class NestedGANCorrection(DeepLearningCorrection):
    """Pipeline processor for Nested-GAN inference on 7-epoch context windows."""

    name = "nested_gan_correction"
    description = "Nested-GAN artifact correction (inner spectrogram + outer time refinement)"
    version = "0.1.0"
