"""Inference integration for the DenoiseMamba (ConvSSD) gradient artifact denoiser."""
...
class DenoiseMambaAdapter(DeepLearningModelAdapter):
    """TorchScript adapter that applies a single-channel ConvSSD denoiser per chunk."""

    spec = DeepLearningModelSpec(
        name="DenoiseMambaAdapter",
        ...
        description="Channel-wise DenoiseMamba (ConvSSD) artifact predictor.",
        tags=("denoise_mamba", "state_space", "mamba", "convssd", "torchscript", "artifact_prediction"),
    )
...
@register_processor
class DenoiseMambaCorrection(DeepLearningCorrection):
    """Pipeline processor for channel-wise DenoiseMamba (ConvSSD) inference."""

    name = "denoise_mamba_correction"
    description = "Channel-wise DenoiseMamba ConvSSD artifact correction"
    version = "0.1.0"
