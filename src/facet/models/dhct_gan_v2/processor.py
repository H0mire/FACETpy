"""Inference integration for the DHCT-GAN v2 multi-epoch context generator."""
...
class DHCTGanV2Adapter(DeepLearningModelAdapter):
    ...
    per scalp channel by extracting the surrounding trigger-to-trigger
    epochs (radius = context_epochs // 2), resamples each to the model-domain
    epoch length, demeans them per epoch, and runs the generator. The
    returned artifact prediction is resampled back to the native center-epoch
    length before :class:`DeepLearningCorrection` subtracts it.
    """

    spec = DeepLearningModelSpec(
        name="DHCTGanV2Adapter",
        ...
        uses_triggers=True,
        description="Multi-epoch context dual-branch hybrid CNN-Transformer GAN artifact predictor.",
        tags=("dhct_gan_v2", "gan", "transformer", "cnn", "context", "torchscript", "artifact_prediction"),
    )
    ...
    def validate_context(self, context: ProcessingContext) -> None:
        ...
        if len(triggers) < self.context_epochs + 1:
            raise ProcessorValidationError(
                f"DHCT-GAN v2 inference requires at least {self.context_epochs + 1} triggers for "
    ...
    def _load_model(self) -> tuple[Any, Any]:
        ...
        try:
            ...
            raise ProcessorValidationError(
                "DHCTGanV2Adapter requires PyTorch. Install the pytorch extra first."
            ) from exc
...
@register_processor
class DHCTGanV2Correction(DeepLearningCorrection):
