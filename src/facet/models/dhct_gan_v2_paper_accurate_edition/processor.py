"""Inference integration for the paper-accurate DHCT-GAN v2 generator.

Mirrors the original ``dhct_gan_v2`` processor exactly (per-channel, 7-epoch
trigger-aligned context, resample/demean/predict loop) but registers under the
GLOBALLY UNIQUE name ``dhct_gan_v2_paper_accurate_correction`` so importing both
editions does not raise a duplicate-registration error.

The exported TorchScript artifact is ``noisy_center - fused_clean``, i.e. the
artifact derived from the full dual-branch + two-mask gating machinery, kept for
the FACETpy subtractive-correction contract (the paper itself emits the fused
clean signal ``Y_pre``).
"""
...
class DHCTGanV2PaperAccurateAdapter(EpochContextArtifactAdapter):
    """Per-channel TorchScript adapter for the paper-accurate DHCT-GAN v2 generator.

    At inference time the adapter builds a ``(context_epochs, samples)`` tensor
    per scalp channel by extracting the surrounding trigger-to-trigger epochs
    (radius = ``context_epochs // 2``), resamples each to the model-domain epoch
    length, demeans them per epoch, and runs the generator. The returned
    artifact prediction is resampled back to the native center-epoch length
    before :class:`DeepLearningCorrection` subtracts it.
    """

    spec = DeepLearningModelSpec(
        name="DHCTGanV2PaperAccurateAdapter",
        ...
        uses_triggers=True,
        description=(
            "Paper-accurate multi-epoch context dual-branch hybrid CNN-Transformer GAN "
            "artifact predictor (LSGAN + feature-matching + three discriminators + two-mask gating)."
        ),
        tags=(
            "dhct_gan_v2_paper_accurate",
            "gan",
            "lsgan",
            "transformer",
            "cnn",
    ...
    def validate_context(self, context: ProcessingContext) -> None:
        ...
        if len(triggers) < self.context_epochs + 1:
            raise ProcessorValidationError(
                f"DHCT-GAN v2 (paper-accurate) inference requires at least {self.context_epochs + 1} "
                f"triggers for {self.context_epochs}-epoch context, got {len(triggers)}"
            )
    ...
    def _load_model(self) -> tuple[Any, Any]:
        ...
        try:
            ...
            raise ProcessorValidationError(
                "DHCTGanV2PaperAccurateAdapter requires PyTorch. Install the pytorch extra first."
            ) from exc
    ...
    def _predict_center_artifact(self, model: Any, torch: Any, epoch_stack: np.ndarray) -> np.ndarray:
        ...
        # Model expects (B, context_epochs, samples).
        ...
        if prediction.ndim != 1:
            raise ProcessorValidationError(
                "DHCT-GAN v2 (paper-accurate) TorchScript model must return shape "
                f"(batch, 1, samples), got {tuple(output.shape)}"
            )
...
@register_processor
class DHCTGanV2PaperAccurateCorrection(DeepLearningCorrection):
    """Pipeline processor for paper-accurate DHCT-GAN v2 per-channel artifact correction."""

    name = "dhct_gan_v2_paper_accurate_correction"
    description = (
        "Paper-accurate multi-epoch context dual-branch hybrid CNN-Transformer GAN "
        "artifact correction (per channel; LSGAN + feature-matching + three discriminators)"
    )
    version = "0.1.0"
    ...
    def __init__(
        ...
        *,
        ...
        adapter = DHCTGanV2PaperAccurateAdapter(
