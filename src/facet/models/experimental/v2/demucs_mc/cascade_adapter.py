...
class CascadeDemucsAdapter(EpochContextArtifactAdapter):
    ...
    def predict(self, context: ProcessingContext) -> DeepLearningPrediction:
        ...
        n_epochs = len(starts)
        centres = range(radius, n_epochs - radius) if self.edge_mode == "skip" else range(n_epochs)
        ...
        resampled = {
            ch: np.stack([_resample_1d(data[ch, a:b], target_samples) for a, b in zip(starts, stops, strict=False)])
            for ch in channels
        }
        ...
        with torch.no_grad():
            for begin in range(0, len(jobs), self.batch_size):
                ...
                stack = np.stack(
                    [
                        np.stack(
                            [resampled[member][index_table[centre]] for member in neighbours[ch]], axis=1
                        )  # (epochs, members, samples)
                ...
                if pred.ndim == 3:
                    pred = pred[:, 0, :]
                for (ch, centre), row in zip(batch, pred, strict=False):
        ...
        metadata = {
            ...
            "skipped_edge_epochs": (min(n_epochs, self.context_epochs - 1) if self.edge_mode == "skip" else 0),
            "edge_epochs_mirrored": (
                0 if self.edge_mode == "skip" else sum(1 for c in centres if c < radius or c >= n_epochs - radius)
            ),
