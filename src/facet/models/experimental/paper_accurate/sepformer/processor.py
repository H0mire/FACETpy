"""Inference integration for the paper-accurate SepFormer context model.

Mirrors ``facet.models.masterthesis.sepformer.processor`` exactly in structure but uses
GLOBALLY UNIQUE registry / adapter names so both editions can be imported in
the same process without a duplicate-registration error.
"""
...
class SepFormerPaperAccurateAdapter(EpochContextArtifactAdapter):
    """TorchScript adapter that runs the paper-accurate SepFormer per channel.

    Consumes a :class:`ProcessingContext` with triggers, builds a
    ``context_epochs``-epoch channel-wise window for every centre TR, runs the
    SepFormer TorchScript model, and produces a centre-epoch artifact estimate
    resampled back to the native epoch length.
    """

    spec = DeepLearningModelSpec(
        name="SepFormerPaperAccurateAdapter",
        ...
        uses_triggers=True,
        description="Paper-accurate SepFormer dual-path Transformer centre-epoch artifact predictor.",
        tags=("sepformer", "paper_accurate", "dual_path_transformer", "context", "torchscript", "artifact_prediction"),
    )
...
@register_processor
class SepFormerPaperAccurateCorrection(DeepLearningCorrection):
    """Pipeline processor for paper-accurate SepFormer context artifact correction."""

    name = "sepformer_paper_accurate_correction"
    description = "Paper-accurate SepFormer dual-path Transformer centre-epoch artifact correction"
    version = "0.1.0"
    ...
    def __init__(
        ...
        *,
        ...
        adapter = SepFormerPaperAccurateAdapter(
