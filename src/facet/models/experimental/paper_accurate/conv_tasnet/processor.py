"""Inference integration for the paper-accurate Conv-TasNet separator.

Mirrors the structure of ``facet.models.masterthesis.conv_tasnet.processor`` but registers
under the GLOBALLY UNIQUE name ``conv_tasnet_paper_accurate_correction`` so the
two editions can coexist without a duplicate-registration error.
"""
...
class ConvTasNetPaperAccurateAdapter(DeepLearningModelAdapter):
    """TorchScript adapter that runs the paper-accurate Conv-TasNet edition.

    Identical channel-wise, fixed-chunk inference contract as the original
    adapter: it consumes ``output[0, 1]`` (the ordered artifact source) and
    returns it as the predicted artifact. The trained checkpoint differs (linear
    encoder + explicit skip width), but the inference shape contract is the same.
    """

    spec = DeepLearningModelSpec(
        name="ConvTasNetPaperAccurateAdapter",
        ...
        description="Channel-wise paper-accurate Conv-TasNet artifact source separator.",
        tags=("conv_tasnet", "paper_accurate", "source_separation", "tcn", "torchscript", "artifact_prediction"),
    )
...
@register_processor
class ConvTasNetPaperAccurateCorrection(DeepLearningCorrection):
    """Pipeline processor for paper-accurate Conv-TasNet artifact separation."""

    name = "conv_tasnet_paper_accurate_correction"
    description = "Channel-wise paper-accurate Conv-TasNet source-separation artifact correction"
    version = "0.1.0"
    ...
    def __init__(
        ...
        *,
        ...
        adapter = ConvTasNetPaperAccurateAdapter(
