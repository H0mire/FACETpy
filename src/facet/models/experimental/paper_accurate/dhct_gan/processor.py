"""Inference integration for the paper-accurate DHCT-GAN generator.

Mirrors ``facet.models.masterthesis.dhct_gan.processor`` exactly, but registers the
globally-unique processor name ``dhct_gan_paper_accurate_correction`` (the
``@register_processor`` registry is global; reusing ``dhct_gan_correction`` would
raise a duplicate-registration error on import). The inference contract is
unchanged: the exported generator returns the artifact head and
:class:`DeepLearningCorrection` subtracts the predicted artifact.
"""
...
class DHCTGanPaperAccurateAdapter(EpochContextArtifactAdapter):
    """Per-channel TorchScript adapter for the paper-accurate DHCT-GAN generator.

    The generator was trained on demeaned, single-channel epoch windows of fixed
    length (default 512 samples). At inference time we extract each
    trigger-to-trigger artifact epoch from the raw signal, resample it to the
    model-domain length, predict the artifact, and resample the prediction back to
    the native epoch length before subtraction by :class:`DeepLearningCorrection`.
    """

    spec = DeepLearningModelSpec(
        name="DHCTGanPaperAccurateAdapter",
        ...
        uses_triggers=True,
        description="Paper-accurate dual-branch hybrid CNN-Transformer GAN artifact predictor (per channel).",
        tags=("dhct_gan", "paper_accurate", "gan", "transformer", "cnn", "torchscript", "artifact_prediction"),
    )
    ...
    def _load_model(self) -> tuple[Any, Any]:
        ...
        try:
            ...
            raise ProcessorValidationError(
                "DHCTGanPaperAccurateAdapter requires PyTorch. Install the pytorch extra first."
            ) from exc
...
@register_processor
class DHCTGanPaperAccurateCorrection(DeepLearningCorrection):
    """Pipeline processor for paper-accurate DHCT-GAN per-channel artifact correction."""

    name = "dhct_gan_paper_accurate_correction"
    description = "Paper-accurate dual-branch hybrid CNN-Transformer GAN artifact correction (per channel)"
    version = "0.1.0"
    ...
    def __init__(
        ...
        *,
        ...
        adapter = DHCTGanPaperAccurateAdapter(
