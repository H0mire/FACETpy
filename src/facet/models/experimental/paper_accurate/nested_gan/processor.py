"""Inference integration for the paper-accurate Nested-GAN generator.

Mirrors ``facet.models.masterthesis.nested_gan.processor`` exactly: a TorchScript adapter
subclassing ``EpochContextArtifactAdapter`` plus a ``@register_processor``
``DeepLearningCorrection`` subclass. The only behavioural differences from the
original are (1) a globally unique registry name
``nested_gan_paper_accurate_correction`` (registry names must be unique, else
import raises a duplicate-registration error) and (2) distinct class names. The
per-channel, per-window resample-in / resample-out / demean / remove-prediction
-mean logic is identical, so the inference contract is unchanged. The exported
TorchScript model is the paper-accurate generator (hierarchical inner Restormer
+ outer time refiner); its forward signature -- ``(B, C_epochs, 1, T) ->
(B, 1, T)`` -- matches the original, so this adapter is a drop-in replacement.
"""
...
class NestedGANPaperAccurateAdapter(EpochContextArtifactAdapter):
    """TorchScript adapter for the paper-accurate Nested-GAN generator.

    Runs the generator per channel per context window: each native epoch is
    resampled to ``epoch_samples``, stacked, fed through the TorchScript model,
    and the predicted center-epoch artifact is resampled back to native length.
    """

    spec = DeepLearningModelSpec(
        name="NestedGANPaperAccurateAdapter",
    ...
    def _load_model(self) -> tuple[Any, Any]:
        ...
        try:
            ...
            raise ProcessorValidationError(
                "NestedGAN (paper-accurate) requires PyTorch. Install the pytorch extra first."
            ) from exc
    ...
    def _predict_center_artifact(self, model: Any, torch: Any, epoch_stack: np.ndarray) -> np.ndarray:
        ...
        if prediction.ndim != 1:
            raise ProcessorValidationError(
                "TorchScript Nested-GAN must return a single center epoch with shape (batch, 1, samples), "
...
@register_processor
class NestedGANPaperAccurateCorrection(DeepLearningCorrection):
    """Pipeline processor for paper-accurate Nested-GAN inference."""

    name = "nested_gan_paper_accurate_correction"
    description = (
        "Paper-accurate Nested-GAN artifact correction "
        "(hierarchical inner spectrogram Restormer + outer time refinement)"
    )
    version = "0.1.0"
    ...
    def __init__(
        ...
        *,
        ...
        adapter = NestedGANPaperAccurateAdapter(
