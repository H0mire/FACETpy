"""Inference integration for the paper-accurate spatiotemporal GNN model.

Mirrors ``st_gnn/processor.py`` but registers a GLOBALLY UNIQUE processor
name (``st_gnn_paper_accurate_correction``) and unique adapter/spec class
names so the package can be imported alongside the original ``st_gnn``
without a duplicate-registration error.
"""
...
from facet.models.experimental.paper_accurate.st_gnn.training import NIAZY_PROOF_FIT_CHANNELS


class PaperAccurateSpatiotemporalGNNAdapter(EpochContextArtifactAdapter):
    """TorchScript adapter that runs the paper-accurate ST-GNN over context epochs."""

    spec = DeepLearningModelSpec(
        name="PaperAccurateSpatiotemporalGNNAdapter",
        ...
        uses_triggers=True,
        description="Paper-accurate multichannel STGCN with EEG-GCNN geodesic electrode graph.",
        tags=("st_gnn", "paper_accurate", "graph", "chebconv", "torchscript", "artifact_prediction"),
    )
    ...
    def _load_model(self) -> tuple[Any, Any]:
        ...
        try:
            ...
            raise ProcessorValidationError(
                "PaperAccurateSpatiotemporalGNN requires PyTorch. Install the pytorch extra first."
            ) from exc
...
@register_processor
class PaperAccurateSpatiotemporalGNNCorrection(DeepLearningCorrection):
    """Pipeline processor for paper-accurate ST-GNN inference."""

    name = "st_gnn_paper_accurate_correction"
    description = "Paper-accurate spatiotemporal graph-network artifact correction"
    version = "0.1.0"
    ...
    def __init__(
        ...
        *,
        ...
        adapter = PaperAccurateSpatiotemporalGNNAdapter(
