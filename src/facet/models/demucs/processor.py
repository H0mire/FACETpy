"""Inference integration for the time-domain Demucs gradient-artifact model."""
...
class DemucsAdapter(EpochContextArtifactAdapter):
    """TorchScript adapter that flattens 7 trigger-defined epochs per channel.
    ...
    spec = DeepLearningModelSpec(
        ...
        uses_triggers=True,
        description="Time-domain Demucs (U-Net + BiLSTM) channel-wise gradient artifact predictor.",
        tags=("demucs", "audio_source_separation", "u_net", "lstm", "torchscript", "artifact_prediction"),
    )
...
@register_processor
class DemucsCorrection(DeepLearningCorrection):
    """Pipeline processor for Demucs gradient-artifact inference."""

    name = "demucs_correction"
    description = "Time-domain Demucs (U-Net + BiLSTM) channel-wise gradient-artifact correction"
    version = "0.1.0"
