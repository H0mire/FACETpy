...
class ChannelSchema:
    ...
    def get_total_channels(self) -> int:
        ...
        return self.eeg_channels + self.eog_channels + self.ecg_channels + self.emg_channels + self.misc_channels
...
def generate_oscillation(
    n_samples: int, sfreq: float, freq_range: tuple[float, float], amplitude: float, n_components: int = 3
) -> np.ndarray:
    """
    ...
    for freq, phase, amp in zip(freqs, phases, amps, strict=False):
...
def generate_blink_artifact(n_samples: int, sfreq: float, blink_rate: float, amplitude: float) -> np.ndarray:
    """
...
def generate_ecg_artifact(n_samples: int, sfreq: float, heart_rate: float, amplitude: float) -> np.ndarray:
    """
...
def generate_saccade_artifact(n_samples: int, sfreq: float, saccade_rate: float, amplitude: float) -> np.ndarray:
    """
    ...
    for saccade_time in saccade_times:
        ...
        if idx + step_duration < n_samples:
            # Rapid transition
            saccade_signal[idx : idx + step_duration] = np.linspace(0, step_amplitude * 1.1, step_duration)
...
def generate_gabor_spike(
    ...
    envelope = np.where(t < 0, np.exp(-(t**2) / (2 * sigma_rise**2)), np.exp(-(t**2) / (2 * sigma_decay**2)))
...
def generate_slow_wave(n_samples: int, sfreq: float, duration_ms: float = 200.0, polarity: int = -1) -> np.ndarray:
    """
...
def generate_spike_wave_complex(
    sfreq: float, spike_params: SpikeParams, amplitude: float = 100.0
    ...
    duration_ms = spike_params.spike_duration_ms * (
        1 + np.random.uniform(-spike_params.duration_variability, spike_params.duration_variability)
    )
    amp = amplitude * (1 + np.random.uniform(-spike_params.amplitude_variability, spike_params.amplitude_variability))
    ...
    if np.random.random() < spike_params.polyspike_probability:
        n_spikes = np.random.randint(spike_params.polyspike_count_range[0], spike_params.polyspike_count_range[1] + 1)
        poly_spike = spike.copy()

        for _i in range(1, n_spikes):
    ...
    if spike_params.slow_wave_enabled:
        sw_duration_ms = spike_params.slow_wave_duration_ms * (1 + np.random.uniform(-0.2, 0.2))
        ...
        slow_wave = (
            generate_slow_wave(n_samples=sw_samples, sfreq=sfreq, duration_ms=sw_duration_ms, polarity=-spike_polarity)
            * sw_amp
        )
...
def generate_epileptic_spikes(
    n_samples: int, sfreq: float, n_channels: int, spike_params: SpikeParams, channel_names: list[str] | None = None
    ...
    for spike_time in spike_times:
        ...
        swc, center_idx = generate_spike_wave_complex(
            sfreq=sfreq, spike_params=spike_params, amplitude=spike_params.spike_amplitude
        )
...
def generate_synthetic_eeg(
    ...
    oscillation_params: OscillationParams | None = None,
    noise_params: NoiseParams | None = None,
    artifact_params: ArtifactParams | None = None,
    ...
    if channel_schema.eog_channels > 0:
        ...
        blink_signal = generate_blink_artifact(
            n_samples, sfreq, artifact_params.blink_rate, artifact_params.blink_amplitude
        )

        saccade_signal = generate_saccade_artifact(
            n_samples, sfreq, artifact_params.saccade_rate, artifact_params.saccade_amplitude
        )
        ...
        frontal_channels = [
            i for i, name in enumerate(eeg_names) if any(x in name.upper() for x in ["FP", "AF", "F1", "F2", "FZ"])
        ]
    ...
    if channel_schema.ecg_channels > 0:
        ...
        ecg_signal = generate_ecg_artifact(n_samples, sfreq, artifact_params.heart_rate, artifact_params.ecg_amplitude)
...
class EEGGenerator(Processor):
    ...
    name = "eeg_generator"
    description = (
        "Generates synthetic EEG data with realistic neural oscillations, artifacts, and optional epileptic spikes."
    )
    requires_raw = False
    ...
    def __init__(
        ...
        duration: float = 10.0,
        channel_schema: ChannelSchema | dict | None = None,
        oscillation_params: OscillationParams | dict | None = None,
        noise_params: NoiseParams | dict | None = None,
        artifact_params: ArtifactParams | dict | None = None,
        spike_params: SpikeParams | dict | None = None,
        random_seed: int | None = None,
        **kwargs,
    ):
    ...
    def process(self, context: ProcessingContext | None) -> ProcessingContext:
        ...
        new_ctx = context.with_raw(raw) if context is not None else ProcessingContext(raw)
