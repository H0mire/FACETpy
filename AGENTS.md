...
- `pip install -e ".[dev]"` – set up an editable install with all dev dependencies (pytest, ruff).
- `pytest` – execute the test suite (adds coverage data via the config in `pytest.ini`).
- `python -m facet.build` – compile the C extension used in artifact correction.
- `sphinx-build -b html docs/source docs/build` – build the documentation locally for spot-checking narrative changes.

Poetry users can still use `poetry install` / `poetry run ...` as before — the `pyproject.toml` remains fully Poetry-compatible.

## Coding Style & Naming Conventions
Follow PEP 8 with four-space indentation and descriptive, snake_case naming for functions, modules, and variables. Classes use PascalCase, and constants stay in ALL_CAPS. Prefer explicit imports from `facet` modules so call sites stay readable. Log diagnostic output with `loguru` for consistency. Run `pytest` before committing so we catch regressions and gather up-to-date coverage details.
